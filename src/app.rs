...
impl AppState {
    ...
    fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
            ...
            load_state_with_outputs(&platform::config_base_dir(), &platform::state_base_dir())
        {
            ...
            for ts in &mut saved.tabs {
                ...
                // still running, so typing a `--resume` would double-launch
                // the session. `adoptable` covers deferred tabs whose
                // adoption happens later in the boot loader.
    ...
    /// Write everything a restart needs — tabs.json, per-tab output, uptime
    /// and energy — synchronously, bypassing throttles. Shared by shutdown
    /// and hot swap (both must land the last tick before the process goes).
...
pub fn run() {
    ...
    // `--handoff <manifest>` on argv, naming the live PTY fds it kept open
    // across the exec. Adopt them BEFORE the reaper and the tab restore
    // below, so restored tabs reattach to their running shells instead of
    // forking fresh ones (see [`crate::hotswap`]).
