...
mod unix {
    ...
    pub fn adopt_manifest(path: &Path) -> usize {
        ...
        for t in &manifest.tabs {
            ...
            // hand-edited manifest) fails fcntl — skip it. Probed on a
            // borrow: owning a number that is not an open fd would abort
            // on drop (close ⇒ EBADF trips the IO-safety check).
            #[allow(unsafe_code)]
            let probe = unsafe { std::os::fd::BorrowedFd::borrow_raw(t.fd) };
            if rustix::io::fcntl_setfd(probe, rustix::io::FdFlags::CLOEXEC).is_err() {
                warn!(
    ...
    mod tests {
        ...
        /// The adoption registry is process-global; tests that insert
        /// into or drain it hold this so a concurrent `close_unclaimed`
        /// can't eat another test's entry mid-flight.
        static REG_LOCK: Mutex<()> = Mutex::new(());

        fn reg_guard() -> std::sync::MutexGuard<'static, ()> {
            REG_LOCK.lock().unwrap_or_else(std::sync::PoisonError::into_inner)
        }
        ...
        fn adopted_pty_reads_child_output_and_reports_exit() {
            ...
            // Best-effort resize: TIOCSWINSZ fails on a pipe and must
            // only warn, never kill the event loop.
            pty.on_resize(ws);
        ...
        #[test]
        fn adopt_from_args_is_zero_without_the_flag() {
            // Same argv constraint as above: no --handoff pair, so the
            // boot-time scan must fall through to "nothing to adopt".
            assert_eq!(adopt_from_args(), 0);
        }

        #[test]
        fn adopt_ignores_missing_and_garbage_manifests() {
            let dir = tmpdir();
            assert_eq!(adopt_manifest(&dir.path().join("absent.json")), 0);
            let p = dir.path().join("garbage.json");
            std::fs::write(&p, b"not json").unwrap();
            assert_eq!(adopt_manifest(&p), 0);
            assert!(!p.exists(), "garbage manifest removed");
        }

        #[test]
        fn adopt_skips_a_fd_the_exec_never_delivered() {
            // Valid manifest, but the fd number is not open in this
            // process (a double adoption would look like this): the
            ...
            assert_eq!(adopt_manifest(&p), 0);
            assert!(!adoptable("ghost"));
        }

        #[test]
        fn close_unclaimed_drains_the_registry() {
            let _reg = reg_guard();
            let dir = tmpdir();
            let fd = leaked_fd(dir.path(), "orphan");
            let p = write_manifest(
                dir.path(),
                &Manifest {
                    handoff_version: HANDOFF_VERSION,
            ...
            assert_eq!(adopt_manifest(&p), 1);
            assert!(adoptable("orphan"));
            close_unclaimed();
            assert!(!adoptable("orphan"));
        }

        #[test]
        fn adopted_pty_pushes_winsize_and_registers_with_a_poller() {
            // A real PTY master this time (bare /dev/ptmx open — no
            // slave needed) so the winsize ioctls actually land.
            let master = File::options().read(true).write(true).open("/dev/ptmx").unwrap();
            let mut child = std::process::Command::new("/bin/sh")
                .arg("-c")
                .arg("sleep 2")
                .spawn()
            ...
            let mut pty = AdoptedPty::adopt(OwnedFd::from(master), child.id(), ws).unwrap();
            // `adopt` pushed the grid size so a changed layout SIGWINCHes.
            let got = rustix::termios::tcgetwinsize(pty.reader()).unwrap();
            ...
            let got = rustix::termios::tcgetwinsize(pty.reader()).unwrap();
            assert_eq!((got.ws_row, got.ws_col), (40, 120));

            // The event-loop registration round trip on both fds.
            let poller = std::sync::Arc::new(Poller::new().unwrap());
            // Safety: both fds live in `pty`, which outlives the
            // registration — deregistered below before drop.
            #[allow(unsafe_code)]
            unsafe {
                pty.register(&poller, Event::readable(TOKEN_PTY), PollMode::Level)
                    .unwrap();
            }
            pty.reregister(&poller, Event::all(TOKEN_PTY), PollMode::Level).unwrap();
            pty.deregister(&poller).unwrap();

            // A foreign child's exit wakes the SIGCHLD pipe, but the
            // scoped waitpid must not report it as ours.
            std::process::Command::new("/bin/true").status().unwrap();
            std::thread::sleep(std::time::Duration::from_millis(50));
            assert!(pty.next_child_event().is_none());

            // Drop HUPs + reaps the sleep child; the handle wait is
            // bookkeeping (ECHILD).
            drop(pty);
            let _ = child.wait();
        }
