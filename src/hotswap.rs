...
/// Flip the freeze flag without a real handoff — test-only, so `frozen()`'s
/// effect on `PtyTap::read` and the `/tabs` `in_handoff` field can be
/// exercised without `exec`ing.
#[cfg(test)]
...
mod unix {
    ...
    mod tests {
        ...
        #[test]
        fn frozen_tracks_the_upgrade_request_flag() {
            // The flag is process-global; restore it whatever we assert.
            let was = super::super::frozen();
            super::super::set_frozen(false);
            assert!(!super::super::frozen());
            super::super::set_frozen(true);
            assert!(super::super::frozen(), "PtyTap::read parks on this");
            super::super::set_frozen(was);
        }
    }
}
