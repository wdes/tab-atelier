...
impl Render for TerminalView {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .on_scroll_wheel(cx.listener(move |this, ev: &ScrollWheelEvent, _window, _cx| {
                let lines = match ev.delta {
                    ScrollDelta::Lines(pt) => -pt.y as i32,
                    ScrollDelta::Pixels(pt) => -(f32::from(pt.y) / 20.0) as i32,
                };
                if lines != 0 {
                    this.scroll(lines);
                }
            ...
            .on_mouse_up(MouseButton::Left, cx.listener(move |_this, _ev: &MouseUpEvent, _window, _cx| {
            }))
            .size_full()
...
impl Element for TerminalElement {
    ...
    fn prepaint(
        ...
        let mut result_lines = Vec::with_capacity(visible_lines);
                ...
                if cell_data.flags.contains(CellFlags::INVERSE) {
                    ...
                    fg = bg_c;
                    {
                        if let Some(last) = bg_runs.last_mut() {
                            if last.col + last.len == c
                                && hsla_eq(last.color, old_fg)
                            {
                                last.len += 1;
                            } else {
