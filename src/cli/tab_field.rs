...
//!
//! `set-context`, `set-assignment`, `set-rehome-status` were ~80 % byte-identical
//! (audit D2): the same `[--tab <id>] [--clear] <value…>` parse loop, the same
//! silent-no-op-outside-a-tab env guard, the same `_TAB_ID` fallback, and the
//! same `POST /tabs/by-id/{id}/{verb}` with a one-field JSON body. This
//! collapses the three into one [`Field`] descriptor + [`post`], with the parse
//! extracted as a PURE, unit-tested [`parse`] (audit C1).
//!
//! `set-status` is deliberately NOT one of these (multi-field body, no `--tab`
//! /`--clear`); it only shares the HTTP tail, factored out as [`send`].
...
///
/// `Err((code, msg))` for a usage error (the caller prints `msg` and exits
/// `code`): a dangling `--tab`, an unknown `--flag`, or nothing to set. `name`
/// is the subcommand name, only used to format the messages.
///
/// # Errors
/// Returns `Err((2, msg))` on any usage error.
...
///
/// Used by [`post`] AND `set-status` (which has its own multi-field parse but
/// the same send). Returns ureq's result verbatim.
///
/// # Errors
...
///
/// parse → env guard → resolve tab → POST. Mirrors the old per-file `run()`
/// exactly (silent no-op outside a tab; `_TAB_ID` fallback; status mapping).
#[must_use]
...
mod tests {
    ...
    fn parse_set_joins_and_trims_value() {
        ...
        // Multiple positionals join with a space; surrounding whitespace trimmed.
    ...
    fn parse_clear_and_help() {
        ...
        // --clear wins over a value (matches the old runners).
        assert_eq!(
    ...
    fn parse_errors_on_empty() {
        // Nothing to set and no --clear → usage error.
        assert_eq!(parse("set-x", &[]).unwrap_err().0, 2);
        // Whitespace-only value is treated as empty too.
