...
// ---------------------------------------------------------------------------
// Storage — the cross-project cold source. Append/parse mirror the blackboard
// (and the task catalogue): one JSON object per line, tolerant readers.
// ---------------------------------------------------------------------------
...
/// line from a racing appender is dropped, not fatal — same tolerance as the blackboard).
#[must_use]
...
/// Append one event to the log (create + append, line-atomic like the blackboard /
...
fn archive_month(unix_secs: u64) -> String {
    ...
    chrono::DateTime::from_timestamp(secs, 0).map_or_else(|| "unknown".to_string(), |dt| dt.format("%Y-%m").to_string())
}
...
pub fn archive_decision(log: &Path, outbox: &Path, id: &str, unix_secs: u64) -> std::io::Result<String> {
    ...
    let archived = DecisionEvent {
        ...
        moves: moved
            .into_iter()
            .map(|(original, archived)| ArchivedMove { original, archived })
            .collect(),
        ..Default::default()
    };
...
// topic (pull-friendly), (b) a targeted tab-input PING to pusher+tichef (push).
// Anti-herd: at most those two targets — NEVER a fan-out. 1st RA2 brick.
// ---------------------------------------------------------------------------
...
/// The REAL tranch-note seam: one durable note on the blackboard's `decisions`
/// topic. Non-fatal — the note is the *pull* fallback, so a blackboard we can't
/// write to must not take the `tranch` down with it.
fn real_note(topic: &str, from: &str, msg: &str) {
    let _ = crate::cli::team::note(Some(topic.to_string()), Some(from.to_string()), msg);
}

/// The REAL tranch-ping seam: resolve `target` (index|uuid|name) against the
/// daemon's `/tabs`, then type `msg` into that tab (`POST /tabs/<idx>/input`) —
/// the same delivery `dispatch --to` uses, so a tranch actually wakes the
/// pusher/tichef's turn instead of only being logged. Best-effort: an
/// undiscoverable daemon / unresolvable target errors so the caller counts it —
/// the durable `decisions` note remains the pull fallback.
///
/// `id` / `at_secs` are unused on main: the MX build keyed aligator's swamp queue
/// on them to dedupe wakes, and main has no such queue, so a re-tranch simply
/// re-delivers.
///
/// # Errors
/// Propagates endpoint-discovery / resolution / input-POST failures.
fn real_tranch_ping(_id: &str, target: &str, msg: &str, _at_secs: u64) -> std::io::Result<()> {
    let ep = crate::cli::share_link::discover_endpoint().map_err(std::io::Error::other)?;
    let (idx, _uuid) = crate::cli::share_link::resolve(&ep, target).map_err(std::io::Error::other)?;
    // Mirrors `share_link::send_input`'s POST verbatim (raw body, octet-stream).
    crate::cli::share_link::agent()
...
pub fn push_event(log: &Path, e: &DecisionEvent, reopen: bool) -> std::io::Result<PushOutcome> {
    let existing: Vec<DecisionEvent> = parse_decisions(&std::fs::read_to_string(log).unwrap_or_default())
        .into_iter()
        .filter(|ev| ev.id == e.id)
        .collect();
    if reopen {
...
fn push(args: &[String]) -> i32 {
    ...
    match push_event(&decisions_path(), &e, reopen) {
        Ok(outcome) => {
            ...
            println!(
                "{}",
                serde_json::json!({ verb: e.id, "outcome": format!("{outcome:?}"), "noop": noop })
            );
            0
        }
        Err(err) => {
...
fn mark(args: &[String], kind: DecisionKind) -> i32 {
    ...
    if rc == 0 && kind == DecisionKind::Tranched {
        ...
        // durable `decisions` note is the pull fallback if the tab ping can't reach.
        ...
        if report.failed > 0 {
            eprintln!(
                "decision tranch: notify reached {:?}, {} target(s) unreachable (note posted as fallback)",
                report.pinged, report.failed
            );
        }
...
mod tests {
    ...
    fn pd1_fold_progression_and_sticky_archive_on_real_fs() {
        ...
        assert!(
            v0.iter().any(|d| d.project.as_deref() == Some("kalpin")),
            "transverse: kalpin too"
        );

        // read → tranched: the STATE transits.
        append_line(log.path(), &ev("ra1c", DecisionKind::Read, 110)).unwrap();
        assert_eq!(
            read_decisions_at(log.path(), false)
                .iter()
                .find(|d| d.id == "ra1c")
                .unwrap()
                .state,
        ...
        assert!(
            read_decisions_at(log.path(), false).iter().all(|d| d.id != "ra1c"),
            "archived hidden by default"
        );
        let all = read_decisions_at(log.path(), true);
        assert_eq!(
            all.iter().find(|d| d.id == "ra1c").unwrap().state,
            DecisionState::Archived,
            "surfaced with --includeArchived"
        );
        ...
        assert!(
            read_decisions_at(log.path(), false).iter().all(|d| d.id != "ra1c"),
            "read/tranched after archive do NOT un-archive (sticky)"
        );
        ...
        let ra1c = reopened
            .iter()
            .find(|d| d.id == "ra1c")
            .expect("re-opened → back in the default list");
        assert_eq!(ra1c.state, DecisionState::Open, "explicit re-open resurrects");
        assert_eq!(
            ra1c.title.as_deref(),
            Some("Deploy RA1c v2"),
            "content axis = latest open wins"
        );
    ...
    fn pd2_reopen_after_tranch_drops_the_stale_verdict() {
        ...
        assert_eq!(
            d.verdict, None,
            "a pre-archive verdict does NOT ride a re-opened decision"
        );
    ...
    fn pd3_archive_moves_bundle_on_disk_and_reopen_restores() {
        ...
        assert_eq!(
            ...
            "the RA1c deploy bundle",
            "bytes preserved by the move"
        );
        ...
        assert_eq!(
            d.files,
            vec![archived.to_string_lossy().into_owned()],
            "view.files points at the archive path"
        );
        ...
        let arch_ev = parse_decisions(&body)
            .into_iter()
            .rev()
            .find(|e| e.id == "ra1c" && e.kind == DecisionKind::Archived)
            .unwrap();
        assert_eq!(
            arch_ev.archived_path.as_deref(),
            Some(dir.as_str()),
            "archived_path = the month dir"
        );
    ...
    fn pd4b_routine_push_never_resurrects_a_settled_decision() {
        ...
        assert!(
            read_decisions_at(log.path(), false).iter().all(|d| d.id != "dep"),
            "settled → hidden from the active list"
        );
        ...
        assert_eq!(
            outcome,
            PushOutcome::NoopSettled,
            "routine push on a settled decision is a no-op"
        );
        // It STAYS archived — no resurrection — and the bundle is NOT back on disk.
        assert!(
            read_decisions_at(log.path(), false).iter().all(|d| d.id != "dep"),
            "still hidden after the routine re-push (not resurrected)"
        );
        assert_eq!(
            read_decisions_at(log.path(), true)
                .iter()
                .find(|d| d.id == "dep")
                .unwrap()
                .state,
        ...
        assert!(
            !bundle.exists(),
            "the archived bundle was NOT resurrected on disk by the routine push"
        );
        // The routine re-push appended NOTHING (log did not grow).
        let archived_events = parse_decisions(&std::fs::read_to_string(log.path()).unwrap())
            .into_iter()
            .filter(|e| e.id == "dep")
            .count();
        assert_eq!(
            archived_events, 3,
            "open + tranched + archived — the routine re-push added no event"
        );
    ...
    fn pd3b_two_decisions_same_basename_same_month_no_clobber() {
        ...
        let arch_b = outbox
            ...
            .join("plan.md");
        assert!(
            arch_a.exists() && arch_b.exists(),
            "both archived plan.md coexist (namespaced by id)"
        );
        assert_eq!(
            std::fs::read_to_string(&arch_a).unwrap(),
            "PLAN OF DECISION A",
            "A's archive holds A's bytes"
        );
        assert_eq!(
            std::fs::read_to_string(&arch_b).unwrap(),
            "PLAN OF DECISION B",
            "B's archive holds B's bytes"
        );
        ...
        assert_eq!(
            std::fs::read_to_string(&plan_a).unwrap(),
            "PLAN OF DECISION A",
            "restore(A) → A's bytes at A's original"
        );
        assert_eq!(
            std::fs::read_to_string(&plan_b).unwrap(),
            "PLAN OF DECISION B",
            "restore(B) → B's bytes at B's original"
        );
        ...
        assert!(
            a_moves[0].archived.ends_with("dec-a/plan.md"),
            "A's mapping points at A's namespaced archive"
        );
    }
    ...
    fn pd3b_restore_never_clobbers_a_recreated_original() {
        ...
        assert_eq!(
            ...
            "NEW recreated bytes",
            "recreated original preserved"
        );
        assert!(
            archived.exists(),
            "the archived copy is kept, not force-moved over the newer file"
        );
    }
    ...
    fn pd4_is_noop_open_dedups_an_identical_rerun_only() {
        ...
        assert!(
            is_noop_open(std::slice::from_ref(&o1), &same),
            "identical re-run on an open decision → no-op (idempotent)"
        );

        // A content change → NOT a no-op (it must append the revision).
        let changed = open("d", "harness", "T2", 2);
        assert!(
            !is_noop_open(std::slice::from_ref(&o1), &changed),
            "changed content → appends"
        );

        // A re-open of a read/tranched/archived decision is a REAL state change.
        assert!(
            !is_noop_open(&[o1.clone(), ev("d", DecisionKind::Read, 3)], &same),
            "re-open of a read decision is meaningful"
        );
        assert!(
    ...
    fn kiosk_detail_rides_the_fold_and_stays_optional() {
        ...
        assert!(
            rich.detail.as_deref().unwrap().contains("Options: A / B"),
            "detail rides the fold verbatim"
        );
        ...
        assert!(
            !plain_json.contains("\"detail\""),
            "a detail-less decision omits the key entirely"
        );
        ...
        assert!(
            !is_noop_open(std::slice::from_ref(&base), &with_detail),
            "adding a detail is a real content change"
        );
        assert!(
    ...
    fn fu1_tranch_notify_targets_pusher_and_tichef_never_fans_out() {
        ...
        assert_eq!(
            msg, "DECISION_TRANCHED id=ra1c verdict=GO from=team-back by=PO",
            "note carries id+verdict+pusher"
        );
        let targets: Vec<String> = pinged.borrow().iter().map(|(t, _)| t.clone()).collect();
        assert_eq!(
            targets,
            vec!["team-back", "tichef"],
            "CIBLÉ: pusher + tichef, exactly — never a fan-out"
        );
        assert!(
            pinged.borrow().iter().all(|(_, m)| m == &msg),
            "the ping carries the same DECISION_TRANCHED marker"
        );
        ...
        assert_eq!(
            *solo.borrow(),
            vec!["tichef"],
            "unknown pusher → tichef only (still no fan-out)"
        );
        assert_eq!(r2.pinged, vec!["tichef"]);
        assert_eq!(
            tranch_notify_msg("d", "NO-GO", None),
            "DECISION_TRANCHED id=d verdict=NO-GO from=- by=PO"
        );

        // (3) a pusher that IS tichef is pinged ONCE (de-dup, still no fan-out).
        assert_eq!(
            tranch_targets(Some("tichef")),
            vec!["tichef"],
            "pusher==tichef → single ping"
        );

        // A failing ping is COUNTED, never propagated (best-effort; the note is the fallback).
        let r3 = emit_tranch_notify(
            "d",
            "GO",
            Some("A"),
            |_t, _f, _m| {},
            |_t, _m| Err(std::io::Error::other("no daemon")),
        );
        assert_eq!(r3.failed, 2, "both pings failed…");
        assert!(r3.pinged.is_empty(), "…none reached");
        assert!(
            r3.note_posted,
            "…but the durable note was still attempted (the pull fallback)"
        );
    }
    ...
    // (3) delivers real input pings to A + tichef — round-tripped through the actual
    // note serialization and a real `/tabs/<idx>/input` POST — WHILE (4) `decision
    // list` still shows the tranched state (the passive log is untouched: NO regression).
    #[test]
    fn fu1_tranch_path_notifies_and_leaves_the_passive_log_intact_on_real_fs() {
        use crate::cli::share_link::with_test_server;
        use crate::cli::team::{BOARD_TEST_LOCK, parse_notes};

        // The blackboard path override is process-global, so this serialises the
        // other board-touching tests — two at once would trade boards mid-assert.
        let _guard = BOARD_TEST_LOCK
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let dir = tempfile::tempdir().expect("tempdir");
        let blackboard = dir.path().join("blackboard.jsonl");
        crate::cli::team::set_blackboard_path(Some(blackboard.clone()));
        ...
        // The tranch → notify STEP the CLI runs, through the REAL seams: the note
        // lands on the blackboard (temp path) and each ping is a `POST
        // /tabs/<idx>/input` against a real in-process daemon seeded with the
        // pusher + tichef. This is the exact wiring `mark` calls.
        let (report, delivered) = with_test_server(|state| {
            state.lock().unwrap_or_else(std::sync::PoisonError::into_inner).tabs = vec![
                crate::api::test_snapshot_tab("id-a", "A"),
                crate::api::test_snapshot_tab("id-t", "tichef"),
            ];
            let report = notify_on_tranch(log.path(), "ra1c", "GO", real_note, |target, msg| {
                real_tranch_ping("ra1c", target, msg, 120)
            });
            let delivered = state
                .lock()
                .unwrap_or_else(std::sync::PoisonError::into_inner)
        ...
        assert_eq!(
            report.pinged,
            vec!["A", "tichef"],
            "the pusher READ FROM THE LOG (A) + tichef were pinged"
        );
        assert_eq!(report.failed, 0);
        ...
        let tranched: Vec<_> = notes
            .iter()
            .filter(|n| n.topic.as_deref() == Some("decisions"))
            .collect();
        assert_eq!(tranched.len(), 1, "exactly one DECISION_TRANCHED note (no fan-out)");
        assert_eq!(
            tranched[0].msg, "DECISION_TRANCHED id=ra1c verdict=GO from=A by=PO",
            "from=A read back from --from + verdict"
        );

        // (3) the daemon really received two input deliveries — to A + tichef, in
        // target order (index 0 = A, index 1 = tichef), each carrying the marker.
        assert_eq!(delivered.len(), 2, "pusher + tichef — CIBLÉ, never a fan-out");
        assert_eq!(
            delivered.iter().map(|(idx, _)| *idx).collect::<Vec<_>>(),
            vec![0, 1],
            "delivered to A (idx 0) then tichef (idx 1)"
        );
        for (_, body) in &delivered {
            let text = String::from_utf8_lossy(body);
            assert!(
                text.starts_with("DECISION_TRANCHED"),
                "carries the tranch marker, got {text:?}"
            );
        }
        ...
        let d = views
            .iter()
            .find(|d| d.id == "ra1c")
            .expect("still listed (tranched, not yet archived)");
        assert_eq!(
            d.state,
            DecisionState::Tranched,
            "the passive log is intact — state readable by all"
        );
    ...
    fn fu1_pusher_rides_the_content_axis_and_is_read_back() {
        ...
        o.from = Some("A".into());
        append_line(log.path(), &o).unwrap();
        let events: Vec<DecisionEvent> = parse_decisions(&std::fs::read_to_string(log.path()).unwrap())
            .into_iter()
            .filter(|e| e.id == "d")
            .collect();
        assert_eq!(
            pusher_of(&events).as_deref(),
            Some("A"),
            "the pusher is read back from the stored --from"
        );
        ...
        assert_eq!(
            pusher_of(&plain),
            None,
            "no --from → no pusher (only tichef would be notified)"
        );

        // `from` is part of the idempotence key: a re-push by a different agent appends.
        let a = {
            let mut e = open("d", "harness", "T", 1);
            e.from = Some("A".into());
            e
        };
        let b = {
            let mut e = open("d", "harness", "T", 2);
            e.from = Some("B".into());
            e
        };
        assert!(
            !is_noop_open(std::slice::from_ref(&a), &b),
            "a different pusher is a real content change"
        );
        let same = {
            let mut e = open("d", "harness", "T", 9);
            e.from = Some("A".into());
            e
        };
        assert!(
            is_noop_open(std::slice::from_ref(&a), &same),
            "same pusher + content → still a no-op"
        );
    }
    ...
    fn summary_rides_content_axis_and_is_part_of_idempotence() {
        ...
        let same = {
            let mut e = open("d", "harness", "Titre gras", 9);
            e.summary = o.summary.clone();
            e
        };
        assert!(is_noop_open(std::slice::from_ref(&o), &same), "same summary → no-op");
        let changed = {
            let mut e = open("d", "harness", "Titre gras", 9);
            e.summary = Some("autre résumé".into());
            e
        };
        assert!(
            !is_noop_open(std::slice::from_ref(&o), &changed),
            "summary change → appends"
        );
    }
    ...
    fn compose_detail_is_deterministic_and_fences_commands() {
        ...
        assert!(
            i_enjeux < i_options && i_options < i_reco && i_reco < i_effort,
            "deterministic order"
        );
        assert!(
            detail.contains("- A: PR maintenant") && detail.contains("- B: attendre"),
            "options list"
        );
        assert!(
            detail.contains("```sh\ngit push origin feat/x\n```"),
        ...
        assert_eq!(
            compose_detail(&["--id".to_string(), "x".to_string()]),
            "",
            "no sections → empty detail"
        );
    }
}
