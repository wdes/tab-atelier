...
///
/// The thin entrypoint the catalogue's `spawn --from-skill`
/// ([`crate::cli::catalog`]) uses to CREATE an agent from a distilled profile
/// (not plan-only). Discovers the local endpoint and defers to
/// [`spawn_agent_tab`] for the create-identify-launch dance.
///
/// # Errors
