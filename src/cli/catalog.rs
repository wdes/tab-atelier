...
impl CatalogCard {
    ...
    pub fn from_snapshot(t: &crate::api::SnapshotTab, after_action: Option<String>, retired_at: u64) -> Self {
        ...
        Self {
            ...
            last_mission: after_action.filter(|s| !s.trim().is_empty()),
            // SV4 (DEFERRED): the tab-side `spawn_mode` field is dashboard-B2 card-model
            // infra not in the agent-card base — the record's A/B partition is stamped at
            // retire via `V2Stamp` (see `with_v2`), so `byMode` stays faithful. When the
            // tab carries a spawn_mode (B2), re-add `spawn_mode: t.spawn_mode` here.
...
pub fn latest_content_for<'a>(records: &'a [CatalogCard], skill: &str) -> Option<&'a CatalogCard> {
    records
        .iter()
        .rfind(|c| c.has_skill() && c.fold_key() == skill && c.kind.is_content())
}
...
pub fn plan_edit(
    latest: Option<&CatalogCard>,
    skill: &str,
    body: &EditBody,
    now: u64,
) -> Result<CatalogCard, EditError> {
    let latest = latest.ok_or(EditError::NotFound)?;
    // Optimistic concurrency (borne 3): a stale expected version → refuse.
    if body
        .prompt_version
        .is_some_and(|expected| latest.prompt_version.unwrap_or(0) != expected)
    {
...
fn spawn_from_skill_run(name: &str, task: Option<&str>, resume: bool) -> i32 {
    ...
    let baseline_owned = resume
        .then(|| resolve_skill_baseline(&read_catalog_cards(), &profile.skill))
        .flatten();
...
mod tests {
    ...
    fn session_id_is_archived_before_close() {
        ...
        assert!(
            matches!(out2, RetireOutcome::Incomplete(_)),
            "lost session → gate refuses"
        );
    ...
    fn idempotent_until_close_on_any_pre_close_failure() {
        ...
        assert!(
            m.log.borrow().is_empty(),
            "write failed → no read-back, no de-register, NO close"
        );
        ...
        assert!(
            m2.log.borrow().is_empty(),
            "de-register failed → shutdown NEVER runs (no ghost)"
        );
    }
    ...
    fn retire_does_not_touch_a_neighbours_card() {
        ...
        assert_eq!(
            read_back(cat.path(), "tab-y"),
            Some(y),
            "neighbour Y's card is untouched"
        );
    ...
    fn deregistered_id_is_absent_and_never_restored() {
        ...
        assert_eq!(
            restored_ids,
            vec!["keep"],
            "restore loop never sees the de-registered id"
        );
    ...
    fn a_completed_retire_does_not_re_add_the_id() {
        ...
        assert!(
            !saved.tabs.iter().any(|t| t.id == "gone"),
            "persist replay never re-adds a retired id"
        );
    }
    ...
    fn rc1_canonical_slug_is_kebab_role_domain_suffix_free() {
        ...
        assert_eq!(
            canonical_slug("Code Reviewer", None),
            "code-reviewer",
            "kebab + lowercase"
        );
        ...
        assert_eq!(
            canonical_slug("builder-2-3", None),
            "builder",
            "stacked suffixes stripped"
        );
    ...
    fn rc1_card_slug_derives_from_assignment_and_specialty() {
        assert_eq!(
            card_slug(Some("build/builder"), None),
            "builder",
            "role from the assignment"
        );
        assert_eq!(
        ...
        assert_eq!(
            card.slug, "builder-rust-daemon-internals",
            "slug computed from the card"
        );
    }
    ...
    fn rc1_dedup_by_slug_folds_latest_and_aggregates_usage() {
        ...
        assert_eq!(
            builder.usage_count,
            Some(8),
            "usageCount aggregated across retraits (3+5)"
        );
    ...
    fn rb2_read_retired_folds_by_slug_keeps_fields_and_is_inert() {
        ...
        for forbidden in ["lease", "\"status\"", "claimedBy", "\"state\""] {
            assert!(
                ...
                "a retired card is inert (no {forbidden}): {json}"
            );
        }
        assert!(
            retired.iter().any(|c| c.slug == "reviewer"),
            "the reviewer profile is its own row"
        );
    }
    ...
    fn rb3_gate_3a_no_ack_safe_to_close_never_closes() {
        ...
        assert!(
            m.log.borrow().is_empty(),
            "no ACK → NOTHING runs (no archive, no close)"
        );
    }
    ...
    fn rb3_crash_between_deregister_and_close_never_leaves_a_ghost() {
        ...
        assert!(
            !saved.borrow().contains(&"gone".to_string()),
            "de-registered BEFORE the crash — no ghost"
        );
        assert!(
            saved.borrow().contains(&"keep".to_string()),
    ...
    fn rb3_deregister_atomic_persists_removal() {
        ...
        assert!(
            !reloaded.tabs.iter().any(|t| t.id == "gone"),
            "id gone from tabs.json (atomic)"
        );
    ...
    fn rb4_resolve_card_by_id_or_slug() {
        ...
        let cards = vec![c];
        assert_eq!(
            resolve_card(&cards, "the-id").map(|c| &c.id),
    ...
    fn rb4_reseed_plan_reposts_four_fields_and_bumps_usage() {
        ...
        assert_eq!(
            plan.specialty.as_deref(),
            Some("rust daemon internals"),
            "specialty re-posted"
        );
        assert_eq!(
        ...
        assert_eq!(
            plan.conventions,
            vec!["CONVENTIONS.md", "memory/index.md"],
            "conventions re-posted"
        );
    ...
    fn rb4_resume_opt_in_reattaches_the_archived_session() {
        ...
        match mode {
            ResumeMode::Resume { command } => {
                assert!(
                    command.contains("sess-abc"),
                    "the resume command carries the archived session id"
                );
            }
    ...
    fn sv3_v1_record_carries_no_v2_fields() {
        ...
        for absent in [
            ...
            assert!(
                ...
                "v1 record must not carry the v2 key {absent}: {json}"
            );
        }
    }
    ...
    fn sv3_fresh_vs_resume_is_derived_not_stored() {
        ...
        assert!(
            (dd - (2.0 / 3.0 - 1.0)).abs() < 1e-9,
            "delivery_delta = fresh_rate − resume_rate"
        );
        assert!(
            (fvr.tokens_ratio.expect("tokens ratio") - 0.2).abs() < 1e-9,
            "tokens_ratio = 200/1000"
        );
        assert!(
            (fvr.cost_ratio.expect("cost ratio") - 0.2).abs() < 1e-9,
            "cost_ratio = 2.0/10.0"
        );
        ...
        for forbidden in ["freshVsResume", "deliveryDelta", "tokensRatio", "costRatio", "byMode"] {
            assert!(
                ...
                "derived field must not be persisted: {forbidden}"
            );
        }
    }
    ...
    fn sv5_proper_name_is_the_stable_fold_key_across_clones() {
        ...
        assert_eq!(
            profiles.len(),
            1,
            "N clones + one proper name → ONE profile (name, not slug, folds)"
        );
        let p = &profiles[0];
        ...
        assert_eq!(
            nameless.fold_key(),
            "fallback-slug",
            "no proper name → stable slug fallback"
        );
    }
    ...
    fn sv1_bilan_four_fields_round_trip_and_replace_last_mission() {
        ...
        assert_eq!(
            card.bilan.as_ref(),
            Some(&bilan),
            "the structured 4-field bilan lands on the card"
        );
        // Replaces lastMission: last_mission is back-filled with a digest of the bilan.
        let lm = card
            .last_mission
            .as_deref()
            .expect("last_mission back-filled from the bilan");
        assert!(
        ...
        assert_eq!(
            (
                b.learned.len(),
                b.problems.len(),
                b.add_directives.len(),
                b.drop_directives.len()
            ),
            (1, 1, 1, 1)
        );
    }
    ...
    fn sv1_bilan_is_prompt_scoped_separate_from_precise_task_context() {
        ...
        assert!(
            !joined.contains("ship RB1"),
            "bilan does not carry the precise objective"
        );
        assert!(
            !joined.contains("step one"),
            "bilan does not carry the precise task log"
        );
        // The precise context lives in its own fields, untouched by the bilan.
        assert_eq!(
            card.objective.as_deref(),
            Some("ship RB1"),
            "precise objective untouched"
        );
        assert_eq!(
            card.current_task_log,
            vec!["step one", "step two"],
            "precise task log untouched"
        );
    }
    ...
    fn sv1_bilan_is_archived_before_close_and_before_eval() {
        ...
        let out = perform_retire(
            ...
            || {
                ...
                assert_eq!(
                    *seen_at_write.borrow(),
                    Some(bilan.clone()),
                    "bilan archived BEFORE close/éval"
                );
                Ok(())
            },
        );
    ...
    fn sv1_empty_bilan_is_a_noop_and_after_action_stands() {
        ...
        assert_eq!(
            card.last_mission.as_deref(),
            Some("legacy 1-line"),
            "the legacy after-action still stands"
        );
    }
    ...
    fn sv2_consensus_clean_improves_prompt_and_derives_outcome() {
        let input = EvalInput {
            ...
            bilan: Bilan {
                add_directives: vec!["always state the invariant up front".into()],
                ..Default::default()
            },
            task_literals: vec![], // clean
            votes: EvalVotes {
                agent: vote(true, true),
                orchestrator: vote(true, true),
                olympe: vote(true, false),
            },
        };
        ...
        assert_eq!(
            r.report.decision,
            EvalDecision::Improved,
            "unanimous approve + clean → improved"
        );
        assert!(
            r.resulting_prompt.contains("always state the invariant up front"),
            "the +directive is in the prompt"
        );
        assert!(
            r.resulting_prompt.contains("be a lazy senior"),
            "the base prompt is kept"
        );
        assert_eq!(
            r.report.outcome,
            Outcome::Success,
            "2/3 run_ok → success (DERIVED, not self-report)"
        );
    ...
    fn sv2_dissent_keeps_statu_quo_traced() {
        let input = EvalInput {
            ...
            bilan: Bilan {
                add_directives: vec!["a general directive".into()],
                ..Default::default()
            },
            votes: EvalVotes {
                agent: vote(true, true),
                orchestrator: vote(false, false),
                olympe: vote(true, false),
            },
            ..Default::default()
        };
        ...
        assert!(
            r.report.rationale.iter().any(|s| s.contains("dissent")),
            "dissent is traced"
        );
    ...
    fn sv2_anti_over_fit_vetoes_leaked_literal_even_on_consensus() {
        let input = EvalInput {
            ...
            bilan: Bilan {
                add_directives: vec![
                    "keep the persist-gate invariant".into(),
                    "remember to ship RB1 first".into(),
                ],
                ..Default::default()
            },
        ...
        assert_eq!(
            ...
            "leaked literal vetoes the improvement"
        );
        assert_eq!(r.resulting_prompt, "base", "over-fit → the original prompt is kept");
        assert!(
            r.report.leaked_literals.iter().any(|l| l == "RB1"),
            "the leaked literal is flagged (E3b)"
        );
        assert!(
        ...
        let general = r
            ...
            .directive_verdicts
            .iter()
            .find(|v| v.directive.contains("persist-gate"))
            .unwrap();
        ...
        let specific = r
            ...
            .directive_verdicts
            .iter()
            .find(|v| v.directive.contains("RB1"))
            .unwrap();
        assert_eq!(
            specific.scope,
            DirectiveScope::TaskSpecific,
            "the leaking directive is task-specific"
        );
    }
    ...
    fn sv2_task_literals_extracted_from_precise_context_not_prose() {
        ...
        assert!(
            !lits.iter().any(|l| l == "ship" || l == "step" || l == "one"),
            "prose words are not literals"
        );
        ...
        assert!(
            !lits2.contains(&"wire".to_string()) && !lits2.contains(&"and".to_string()),
            "prose skipped"
        );
    }
    ...
    fn sv2_cf1_v2_needs_skill_and_prompt_session_optional_and_gates_the_close() {
        ...
        assert!(
            ...
            "CF1: v2 with skill+prompt is complete even had_session + no session_id"
        );
        ...
        assert!(
            matches!(out, RetireOutcome::Incomplete(_)),
            "CF1: a v2 retire never closes without skill+prompt"
        );
    }
    ...
    fn sv2_eval_derived_outcome_overrides_self_report() {
        ...
        let result = evaluate(&EvalInput {
            base_prompt: "p".into(),
            votes: EvalVotes {
                agent: vote(false, false),
                orchestrator: vote(false, true),
                olympe: vote(false, false),
            },
            ..Default::default()
        });
        ...
        assert_eq!(
            evaled.outcome,
            Some(Outcome::Problem),
            "the eval-derived outcome overrides the self-report"
        );
        assert!(
            evaled.eval.is_some(),
            "the eval report is stored (FN1 verdicts + FN2 findings + trace)"
        );
    }
    ...
    fn sv4_resolve_skill_profile_matches_by_name_not_slug() {
        ...
        assert!(
            resolve_skill_profile(&profiles, "builder-rust-daemon").is_none(),
            "a slug never matches (🟡 ii)"
        );
    ...
    fn sv4_plan_default_is_fresh_with_task_overlay() {
        ...
        assert!(
            plan.prompt.contains("be a lazy senior"),
            "the profile prompt is the base"
        );
        assert!(
            plan.prompt.contains("Task: fix the flaky test"),
            "the --task overlay is appended"
        );
        ...
        assert_eq!(
            plan_from_skill(&p, None, None, false).unwrap().prompt,
            "be a lazy senior"
        );
    }
    ...
    fn sv4_plan_resume_reuses_baseline_session() {
        ...
        assert!(
            plan.cmd.contains("sess-champion"),
            "the resume command carries baseline.sessionId: {}",
            plan.cmd
        );
        ...
        assert!(
            plan_from_skill(&p, None, None, true).is_err(),
            "--resume needs a baseline"
        );
    }
    ...
    fn sv4_resolve_skill_baseline_picks_latest_session_of_the_skill() {
        ...
        assert!(
            resolve_skill_baseline(&cards, "no-such-skill").is_none(),
            "no session → no baseline"
        );
    }
    ...
    fn sv5_g1_min_sample_gates_the_verdict() {
        ...
        assert_eq!(
            fvr.verdict,
            AbVerdict::InsufficientSample,
            "resume arm under MIN → G1 gates the verdict"
        );
    ...
    fn sv5_g3_directional_verdict_surfaced_with_n() {
        ...
        assert_eq!(
            fvr.verdict,
            AbVerdict::FreshFavored,
            "fresh 1.0 vs resume 0.0 → fresh favored"
        );
        assert_eq!(
            (fvr.fresh_n, fvr.resume_n),
            (MIN_SAMPLE, MIN_SAMPLE),
            "n surfaced with the trend (G3)"
        );
    ...
    fn sv5_g2_verdict_is_within_skill_not_cross_skill() {
        ...
        assert_eq!(
            a.fresh_vs_resume.verdict,
            AbVerdict::FreshFavored,
            "skill-a judged on its own arms"
        );
        assert_eq!(
            b.fresh_vs_resume.verdict,
            AbVerdict::ResumeFavored,
            "skill-b judged on its own arms"
        );
    }
    ...
    fn sv5_tokens_reuse_telemetry_and_stamp_can_override() {
        ...
        let base_stamp = V2Stamp {
            skill: Some("builder".into()),
            prompt: Some("p".into()),
            ..Default::default()
        };
        assert_eq!(
        ...
        assert_eq!(
            card.with_v2(override_stamp).tokens,
            Some(9999),
            "an explicit stamp figure overrides"
        );
    }
    ...
    fn sc1_retire_record_omits_the_kind_key() {
        ...
        assert!(
            !json.contains("\"kind\""),
            "a retire record omits the kind key (byte-identical): {json}"
        );
        ...
        assert!(
            encode_catalog_line(&edit).contains(r#""kind":"edit""#),
            "an edit carries kind:edit"
        );
    }
    ...
    fn sc1_content_is_latest_append_not_latest_timestamp() {
        ...
        assert_eq!(
            p.prompt.as_deref(),
            Some("edited prompt"),
            "the later-APPENDED edit wins (append order)"
        );
        assert_eq!(p.prompt_version, Some(2));
    }
    ...
    fn sc1_delete_is_sticky_only_restore_resurrects() {
        ...
        assert!(
            read_skill_profiles_at(cat.path()).is_empty(),
            "delete tombstones the skill"
        );
        ...
        assert!(
            read_skill_profiles_at(cat.path()).is_empty(),
            "edit after delete does NOT resurrect (borne 5)"
        );
        ...
        assert!(
            read_skill_profiles_at(cat.path()).is_empty(),
            "retire after delete does NOT resurrect"
        );
        ...
        assert_eq!(
            after[0].prompt_version,
            Some(3),
            "restored with the latest content (append order)"
        );
    }
    ...
    fn sc1_plan_edit_carries_bumps_and_gates() {
        ...
        assert_eq!(
            plan_edit(Some(&latest), "rustsmith", &empty, 100),
        ...
        assert_eq!(
            plan_edit(Some(&latest), "rustsmith", &stale, 100),
        ...
        assert!(
            plan_edit(Some(&latest), "rustsmith", &ok, 100).is_ok(),
            "matching version → ok"
        );
    ...
    fn sc1b_read_all_marks_deleted_and_default_hides_them() {
        ...
        assert!(
            serde_json::to_string(gone).unwrap().contains(r#""deleted":true"#),
            "camelCase marker present"
        );
        assert!(
            !serde_json::to_string(v).unwrap().contains("deleted"),
            "no marker on a visible card"
        );
    }
}
