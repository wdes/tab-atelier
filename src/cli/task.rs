...
// A per-thread tasks-dir override for tests: the route-handler integration
// tests isolate their queue files onto a tmpdir WITHOUT `env::set_var` (which is
// `unsafe` under edition 2024 + `-D unsafe-code`, and races across cargo's test
// threads). Thread-local so each test — and the handler it drives on the SAME
// thread — sees its own dir with no cross-test contention.
#[cfg(test)]
thread_local! {
    static TASKS_DIR_OVERRIDE: std::cell::RefCell<Option<PathBuf>> = const { std::cell::RefCell::new(None) };
}

/// Point this thread's [`tasks_dir`] at `dir` for the duration of a test.
#[cfg(test)]
pub fn set_tasks_dir_for_test(dir: PathBuf) {
    TASKS_DIR_OVERRIDE.with(|c| *c.borrow_mut() = Some(dir));
}

/// The directory holding the per-queue files: `<state>/tab-atelier/tasks` in
/// production; a per-thread tmpdir override under `#[cfg(test)]`.
#[must_use]
pub fn tasks_dir() -> PathBuf {
    #[cfg(test)]
    if let Some(d) = TASKS_DIR_OVERRIDE.with(|c| c.borrow().clone()) {
        return d;
    }
