...
fn spawn_pty_tab(
    ...
    Some(HeadlessTab {
        ...
        parent_tab_id: None,
        rehome_status: None,
        // Card fields restored from TabState at the call site (like `assignment`).
        specialty: None,
        orchestrator: None,
        objective: None,
...
pub fn run() -> std::io::Result<()> {
    ...
    // POST is untouched). BEST-EFFORT / non-blocking; skipped in read-only. Reduces
    // the restart→orchestrator latency from the cron watcher's 5-14 min to one
    // aligator tour. Shared with the gui daemon via `emit_startup_wake` (RA1b).
    {
...
fn refresh_snapshot(
    ...
    for tab in tabs.iter_mut() {
        ...
        api_tabs.push(api::SnapshotTab {
            ...
            resident_memory_bytes: crate::agent_probe::sample_tree_cached(tab.pid)
...
fn drain_pending(
    ...
    // …and the Inc8 agent-card mutations (persisted like assignment). Overwrite
    // fields set/clear; current-task APPENDS to the bounded permalog; rounds-active
    // sets the supervision status. (The snapshot mirror was already updated in the
    // route; this writes the runtime tab so the next persist tick hits tabs.json.)
