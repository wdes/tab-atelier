...
//     https://example.org/decisions/file?path=<report>&token=<the page token> (the shareable off-LAN URL).
...
async function main() {
  ...
  ok("(3) ⭐ the remote href is the ABSOLUTE amaury viewer URL", remoteHref.startsWith("https://example.org/decisions/file?path="), `remoteHref=${remoteHref}`);
  ...
  console.log(`${failures ? `FAIL: ${failures} assertion(s) failed` : "OK: report row keeps its intact local viewer link AND gains an 'Ouvrir en distant' link whose REAL DOM href is https://example.org/decisions/file?path=<report>&token=<page token>"}`);
