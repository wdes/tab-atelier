...
// loopback AND behind a public host like example.org). No url/token → passthrough.
...
// Testability seam (dashboard-aggregation PR): the two feature-detected panels'
// open() paths are exported so assets/dashboard.degradation.test.mjs can prove
// the graceful degradation is WIRED end to end — a 404 from the (not-yet-landed)
// /decisions or /catalog/list route renders an "indisponible" state with NO
// crash, a 200 populates. This is the ⭐ anti-built≠wired check: the panels are
// real, tested code, not dead code hoping the backend appears.
export { openKiosk, openCatalog };
