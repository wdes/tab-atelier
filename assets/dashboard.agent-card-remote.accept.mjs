...
//   ⭐ the card carries an "⇱ distant (écriture)" link whose REAL href = https://example.org/tabs/by-id/<uuid>/view?token=<page token>
...
async function main() {
  ...
  ok("(2) ⭐ the remote href is the ABSOLUTE amaury TAB-viewer URL", remoteHref.startsWith(`https://example.org/tabs/by-id/${AID}/view`), `remoteHref=${remoteHref}`);
  ...
  console.log(`${failures ? `FAIL: ${failures} assertion(s) failed` : "OK: the agent-card gains a WRITE remote link (real DOM href https://example.org/tabs/by-id/<uuid>/view?token=<page token>, rel=noreferrer) while the ↗ local viewer AND free-zone right-click stay LOCAL (zero regression)"}`);
