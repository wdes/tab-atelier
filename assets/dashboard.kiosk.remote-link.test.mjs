...
// COMPOSED into a shareable ABSOLUTE url (REMOTE_BASE = example.org) so the PO can open it off-LAN
...
// ============================ toRemoteLink — compose the shareable remote URL ============================
{
  ...
  assert.ok(u.startsWith("https://example.org"), "starts with the fixed remote host (default amaury)");
  ...
  assert.ok(noTok.startsWith("https://example.org/decisions/file?path=outbox/x.md"), "tokenless → still absolute + pathed");
