/**
 * Unicode 11 width provider for xterm.js — vendored, self-contained port of
 * @xterm/addon-unicode11 at the xterm.js 6.0.0 tag (matches vendor/xterm-6.0.0).
 *
 * Copyright (c) 2019 The xterm.js authors. All rights reserved. @license MIT
 * Source: xtermjs/xterm.js addons/addon-unicode11/src/UnicodeV11.ts (tag 6.0.0)
 *
 * Why we ship this: xterm.js defaults to the Unicode 6 width tables, where
 * emoji are 1 cell wide. The server grid (alacritty) advances the cursor by 2
 * for emoji, so with V6 the viewer clips every emoji to half a cell and the
 * columns desync (see the "emoji cut in half" bug). Activating V11 makes the
 * viewer count emoji as 2 cells, matching the desktop terminal.
 *
 * Exposes `window.Unicode11Addon` — a standard xterm addon. Load with
 * `term.loadAddon(new Unicode11Addon()); term.unicode.activeVersion = '11';`.
 *
 * The `charProperties` bit-packing constants are inlined from the core's
 * `UnicodeService` (createPropertyValue/extractWidth) so the file needs no
 * xterm imports; they are ABI-locked to the vendored 6.0.0 core.
 */
(function () {
  "use strict";
  ...
  // BMP lookup table, lazily initialised the first time a provider is built.
  var table;
  ...
  // Bit-packing mirror of the core UnicodeService (ABI-locked to xterm 6.0.0):
  //   createPropertyValue(charKind, width, shouldJoin)
  //     = (charKind & 0xFFFFFF) << 3 | (width & 3) << 1 | (shouldJoin ? 1 : 0)
  //   extractWidth(v) = (v >> 1) & 3
  function extractWidth(v) {
    return (v >> 1) & 3;
  }
  function createPropertyValue(charKind, width, shouldJoin) {
  ...
  function UnicodeV11() {
    ...
    if (!table) {
      ...
      for (var i = 0; i < BMP_COMBINING.length; ++i) {
        table.fill(0, BMP_COMBINING[i][0], BMP_COMBINING[i][1] + 1);
      }
      for (var j = 0; j < BMP_WIDE.length; ++j) {
        table.fill(2, BMP_WIDE[j][0], BMP_WIDE[j][1] + 1);
      }
    }
  }
  UnicodeV11.prototype.wcwidth = function (num) {
    if (num < 32) return 0;
    if (num < 127) return 1;
  ...
  UnicodeV11.prototype.charProperties = function (codepoint, preceding) {
    var width = this.wcwidth(codepoint);
    var shouldJoin = width === 0 && preceding !== 0;
    if (shouldJoin) {
      var oldWidth = extractWidth(preceding);
      if (oldWidth === 0) {
        shouldJoin = false;
  ...
  // Standard xterm addon wrapper: `term.loadAddon(new Unicode11Addon())`.
  function Unicode11Addon() {}
  Unicode11Addon.prototype.activate = function (terminal) {
    terminal.unicode.register(new UnicodeV11());
  };
  Unicode11Addon.prototype.dispose = function () {};
