...
///
/// `NO_COLOR` is the cross-tool convention: present and non-empty disables
/// colour *whatever its value*, so `NO_COLOR=0` still means off. That reads as a
/// bug until you know the rule — the point is that an operator does not have to
/// remember which spelling a given tool wants — so it is pinned in the tests
/// below rather than left to be "fixed" later.
///
/// `CLICOLOR=0` is the older BSD-style spelling. tab-atelier's `new_tab_env`
/// sets both together for the tabs an agent asked for, since those tabs' output
/// is read by another program (`peek`, `output`, a `--wait` poll) — so honouring
/// only one would leave the other looking ignored.
///
/// `TERM` is deliberately not consulted, matching the app: its right-click
/// "Disable colors" sets `TERM=dumb` for the shell, and reading that as a colour
/// hint would take an agent's TUI apart.
#[must_use]
pub fn colour_disabled(no_color: Option<&str>, clicolor: Option<&str>) -> bool {
    let no_color = no_color.is_some_and(|v| !v.is_empty());
    let clicolor_off = clicolor.is_some_and(|v| v.trim() == "0");
    no_color || clicolor_off
}
...
pub fn colour_disabled_in_env() -> bool {
    ...
    colour_disabled(no_color.as_deref(), clicolor.as_deref())
}
...
mod tests {
    ...
    fn no_color_disables_on_any_non_empty_value() {
        ...
        assert!(colour_disabled(Some("0"), None));
        assert!(colour_disabled(Some("1"), None));
        assert!(colour_disabled(Some("false"), None));
        assert!(colour_disabled(Some("anything"), None));
    }
    ...
    fn an_unset_or_empty_no_color_does_not_disable() {
        ...
        assert!(!colour_disabled(None, None));
        assert!(!colour_disabled(Some(""), None));
    }

    #[test]
    fn clicolor_zero_disables_and_other_values_do_not() {
        // The BSD-style spelling tab-atelier sets alongside NO_COLOR.
        assert!(colour_disabled(None, Some("0")));
        assert!(!colour_disabled(None, Some("1")));
        assert!(!colour_disabled(None, Some("")));
        assert!(!colour_disabled(None, Some("false")));
    }
