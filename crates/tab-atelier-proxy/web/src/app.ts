 ...
 * re-sent as current. */
function providerModels(p: ProviderView): string {
  return p.models
...
const AdminApp = Vue.defineComponent({
  ...
  computed: {
    ...
    tokenUnits(): { id: string; label: string }[] {
    ...
    tiles(): { label: string; value: string; sub: string }[] {
      ...
      const priced = this.series.some((b: UsageBucket) => b.cost_in_micro != null);
      const paid = sum((b: UsageBucket) => (b.cost_in_micro ?? 0) + (b.cost_out_micro ?? 0));
      return [
    ...
    modelChoices(): Array<{ value: string; label: string }> {
      ...
      for (const p of all) {
        ...
        for (const m of p.models) {
          if (m.deprecated || seen.has(m.id)) continue;
          seen.add(m.id);
     ...
     */
    priceLabel(m: ProviderModel): string {
      if (!m.price) return "";
      const rate = (micro: number): string => {
  ...
  methods: {
    ...
    async setUserModel(u: ApiUser, value: string) {
      this.busy = true;
      try {
    ...
    userModelValue(u: ApiUser): string {
      if (!u.model) return "";
      if (this.wireOf(u) !== "openai") return u.model;
    ...
    peakUntil(p: ProviderView): string {
      if (!p.peak_until) return "";
    ...
    tokenSummary(c: Capture): string {
      ...
      if (t.cache_read) parts.push(`cache read ${this.fmt(t.cache_read)}`);
      if (t.cache_write) {
      ...
      if (t.service_tier) parts.push(`tier ${t.service_tier}`);
      if (t.web_search) parts.push(`web search ×${this.fmt(t.web_search)}`);
      if (t.web_fetch) parts.push(`web fetch ×${this.fmt(t.web_fetch)}`);
      return parts.join(" · ");
    },
    async loadInspect() {
     ...
     */
    serverToolLabel(c: Capture): string {
