...
function energyNote(): string {
  return (
    `Estimated from token counts using ${SOURCE_ENERGY.value} ${SOURCE_ENERGY.unit} ` +
...
function moneyNote(): string {
  return (
    `What each hour was charged, recorded by the server as the tokens were used ` +
...
function fmtCount(n: number): string {
  ...
  if (!Number.isInteger(n)) return n < 1 ? n.toFixed(2) : n.toFixed(1);
  return String(n);
}
...
const CallsChart = Vue.defineComponent({
  ...
  props: {
     ...
     */
    byUser: { type: Array as () => readonly UserSeries[], default: () => [] },
  },
  computed: {
    ...
    users(): readonly UserSeries[] {
      return this.byUser as readonly UserSeries[];
    },
    multi(): boolean {
    ...
    // breaks the cycle.
    seriesList(): { key: string; name: string; slot: number; points: readonly UsageBucket[] }[] {
    ...
    seriesPaths(): { key: string; name: string; slot: number; d: string; points: readonly UsageBucket[] }[] {
    ...
    peak(): number {
      if (!this.multi) return Math.max(1, ...this.pts.map((p) => p.calls));
    ...
    hoverRows(): { name: string; slot: number; calls: number; errors: number }[] {
