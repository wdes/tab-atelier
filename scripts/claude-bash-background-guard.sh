...
if printf '%s' "$cmd" | grep -qE '(^|[^[:alnum:]_-])sleep[[:space:]]'; then
...
if printf '%s' "$cmd" | grep -qE 'gh[[:space:]]+run[[:space:]]+watch|--watch|tail[[:space:]]+-[A-Za-z]*f|(^|[;&|(]|&&|\|\|)[[:space:]]*wait([[:space:]]|$|;)'; then
...
if printf '%s' "$cmd" | grep -qE '(^|[;&|(]|&&|\|\|)[[:space:]]*(cargo[[:space:]]+(\+[^[:space:]]+[[:space:]]+)?|bun[[:space:]]+|npm[[:space:]]+|pnpm[[:space:]]+|yarn[[:space:]]+)(run[[:space:]]+)?(build|test|clippy|deb|apk2|check|install)\b'; then
